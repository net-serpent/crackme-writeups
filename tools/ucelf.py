...
class Emu:
    ...
    def __init__(self, path, base=BASE):
        ...
        with open(path, 'rb') as f:
            loads = [s for s in ELFFile(f).iter_segments() if s['p_type'] == 'PT_LOAD']
        ...
        uc.mem_write(self.FS_BASE + 0x28, b'\x88\x77\x66\x55\x44\x33\x22\x11')  # stack canary
    ...
    def _hook(self, uc, addr, size, _):
        ...
        ret_to = self.read_u64(rsp)
        uc.reg_write(UC_X86_REG_RSP, rsp + 8)
        rv = fn(self)
        if rv is not None:
            uc.reg_write(UC_X86_REG_RAX, rv & (2**64 - 1))
        uc.reg_write(UC_X86_REG_RIP, ret_to)
